<template>
  <div v-if="product">
    ...
    <div class="detail-section mt24">
      ...
      <el-form :model="review" label-width="80px" class="mb16 review-form">
      ...
      <el-timeline v-else>
        <el-timeline-item v-for="r in reviews" :key="r.id" :timestamp="formatDate(r.created_at)">
          ...
          <div class="review-text">{{ r.comment || t('msg.noText') }}</div>
        </el-timeline-item>
      </el-timeline>
    </div>
  </div>
...
.review-text{ white-space: pre-wrap; color: #3c4a66; }
