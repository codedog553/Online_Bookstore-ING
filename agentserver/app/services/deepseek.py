...
class DeepSeekService:
    ...
    async def plan_chat(self, message: str, history: List[Dict[str, str]]) -> Dict[str, Any]:
        if not self.enabled:
    ...
    def _fallback_plan(self, message: str) -> Dict[str, Any]:
        ...
        if self._is_identity_question(message) or self._is_greeting(message) or any(word in lowered for word in ["不开心", "难过", "烦", "心情"]):
            return {
                "intent": "general_chat",
                "query": "",
                "tone": "supportive" if any(word in lowered for word in ["不开心", "难过", "烦", "心情"]) else "friendly",
                "needs_catalog": False,
                "needs_follow_up": False,
                "follow_up_question": "",
                "reason": "fallback-general-chat",
            }
        if any(word in lowered for word in ["购物车", "加入", "加购", "删除", "移除", "修改数量", "更新数量"]):
            return {
                "intent": "cart_action",
                "query": "",
                "tone": "friendly",
        ...
        if any(word in lowered for word in ["推荐", "想看", "想要", "给我", "相关", "有没有", "适合"]):
            return {
    ...
    async def suggest_cart_action(self, message: str, history: List[Dict[str, str]], references: List[Dict[str, Any]]) -> Dict[str, Any]:
        if not self.enabled:
            return self._fallback_cart_action(message, references)
        prompt = (
            "你是购物车动作规划器。"
            ...
            "3. 如果信息不足，action 用 none，并在 user_message 中自然追问；"
            "4. 如果用户只是问购物车里有什么，用 action=list；"
            "5. 不允许涉及支付、订单、他人数据或 admin。"
            "6. 当用户表达了明确购书意愿（如 想买/要买/加入购物车/买一本）时，如果 references 中能定位到书，优先输出 add 动作；"
            "7. 当用户只是聊天、咨询、推荐，不要输出 cart 动作。"
            "8. 如果用户一次想买多个版本/规格，可在 sku_requests 中返回多个对象，格式为 [{\"sku_id\":123,\"quantity\":1}]。"
        )
        payload = await self._chat(
            [
                {"role": "system", "content": prompt},
    ...
    def _fallback_cart_action(self, message: str, references: List[Dict[str, Any]]) -> Dict[str, Any]:
        lowered = (message or "").lower()
        if any(word in lowered for word in ["购物车", "看看购物车", "购物车里", "购物车有什么"]):
            return {
                "should_act": True,
                "action": "list",
                ...
                "item_id": None,
                "quantity": None,
                "user_message": "我来帮你查看当前购物车。",
                "missing_fields": [],
            }
        if any(word in lowered for word in ["加入购物车", "加购", "买一", "买一本", "想买"]):
            ...
            if not sku_requests:
                return {
                    ...
                    "action": "none",
                    "requires_confirmation": False,
                    "product_title": str(first.get("title") or "").strip(),
                    "sku_id": None,
                    "sku_requests": [],
                    "item_id": None,
                    "quantity": None,
                    "user_message": "我还缺少你要购买的具体版本或规格，请先告诉我选项后我再帮你确认。",
            ...
            return {
                ...
                "user_message": "我理解你想购买这本书。我会先弹出确认框，只有你确认后才会加入当前会话用户的购物车。",
                "missing_fields": [],
            }
        return {
            "should_act": False,
    ...
    def has_explicit_purchase_intent(self, message: str) -> bool:
        lowered = (message or "").lower()
        return any(word in lowered for word in ["想买", "要买", "买一本", "买一", "加入购物车", "加购", "放购物车"])

    def resolve_cart_item_action(self, message: str, suggestion: Dict[str, Any], cart_items: List[Dict[str, Any]]) -> Dict[str, Any]:
        lowered = (message or "").lower()
        if not cart_items:
            return suggestion
        ...
        target = pick_target()
        if any(word in lowered for word in ["购物车里有什么", "看看购物车", "查看购物车", "我的购物车"]):
            return {
                "should_act": True,
                "action": "list",
                "requires_confirmation": False,
        ...
        if any(word in lowered for word in ["删除", "移除", "不要了", "去掉"]):
            ...
            return {
                ...
                "quantity": None,
                "user_message": "我已经识别到你要删除购物车项，接下来会通过弹窗请你确认。",
                "missing_fields": [],
            }
        ...
        if any(word in lowered for word in ["改成", "改为", "改到", "更新", "数量改", "增加", "减少", "加一", "减一", "两本", "2本", "一本", "1本"]):
            ...
            quantity = None
            if any(word in lowered for word in ["减一", "少一"]) or ("减少" in lowered and "一本" in lowered):
                quantity = max(int(target.get("quantity") or 1) - 1, 0)
            elif any(word in lowered for word in ["加一", "多一"]) or ("增加" in lowered and "一本" in lowered):
            ...
            return {
                ...
                "item_id": target.get("item_id"),
                "quantity": quantity,
                "user_message": "我已经识别到你要修改购物车数量，接下来会通过弹窗请你确认。" if quantity > 0 else "数量调整为 0 会删除该购物车项，接下来会通过弹窗请你确认。",
                "missing_fields": [],
            }
    ...
    def _extract_sku_requests(self, message: str, reference: Dict[str, Any]) -> List[Dict[str, int]]:
        ...
        if not skus:
            ...
            return [{"sku_id": int(sku_ids[0]), "quantity": 1}] if sku_ids else []

        lowered = (message or "").lower()
        requests: List[Dict[str, int]] = []
        matched_ids = set()

        def quantity_for(keyword: str) -> int:
            if keyword in lowered:
                if f"{keyword}两本" in lowered or f"{keyword}2本" in lowered:
                    return 2
                return 1
            return 0
        ...
        for sku in skus:
            ...
            if not sku_id or not sku.get("is_available", True):
                continue
            qty = 0
            if "精装" in option_text:
                qty = quantity_for("精装")
            elif "平装" in option_text:
                qty = quantity_for("平装")
            if qty > 0:
        ...
        for sku in skus:
            ...
            if sku_id and int(sku_id) not in matched_ids and sku.get("is_available", True):
                return [{"sku_id": int(sku_id), "quantity": 1}]
        return []
    ...
    def _wants_recommendation(message: str) -> bool:
        value = (message or "").lower()
        return any(keyword in value for keyword in ["推荐", "想要", "给我", "类似", "相关", "还有", "其他", "别的", "找一本", "有没有"])

    @staticmethod
    def _is_follow_up(message: str) -> bool:
        value = (message or "").lower()
        return any(keyword in value for keyword in ["还有", "其他", "别的", "换一本", "类似", "除此之外", "除了"])

    @staticmethod
    def _short_desc(item: Dict[str, Any], size: int = 34) -> str:
    ...
    def _theme_candidates(self, focus: str, catalog_snapshot: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for item in catalog_snapshot:
            ...
            score = 0
            if any(word in lowered_focus for word in ["编程", "计算机", "python", "开发", "程序", "代码", "算法", "软件"]):
            ...
            if any(word in lowered_focus for word in ["科幻", "science fiction", "sci-fi"]):
                ...
                if any(word in author for word in ["刘慈欣", "阿西莫夫", "verne", "asimov", "orwell", "玛丽", "雪莱"]):
                    score += 4
            if "反乌托邦" in lowered_focus:
                if any(word in desc + " " + desc_en + " " + title for word in ["极权", "监视", "控制", "社会", "反乌托邦"]):
                    score += 10
            if score > 0:
    ...
    async def extract_book_query(self, message: str, history: List[Dict[str, str]]) -> str:
    ...
    async def compose_book_answer(
        ...
        if not books and not self.enabled:
            return self._fallback_answer(message, history, plan, books, catalog_snapshot)
        if not self.enabled:
            return self._fallback_answer(message, history, plan, books, catalog_snapshot)
        prompt = (
            "你是在线书店智能助手。"
    ...
    def _fallback_answer(
        ...
        if self._is_identity_question(message):
            return "我是在线书店智能助手，可以陪你聊天，也可以查书、介绍书籍信息，并按作者、题材或关键词推荐。"
        if self._is_greeting(message):
            return "你好，我是在线书店智能助手。你可以问我书籍信息、作者、题材，或者让我直接推荐。"
        if intent == "general_chat" and any(word in message for word in ["不开心", "难过", "烦", "心情"]):
            theme_candidates = self._theme_candidates("治愈 温暖 轻松", catalog_snapshot)
            if theme_candidates:
                picks = "；".join([f"《{item['title']}》：{self._short_desc(item)}" for item in theme_candidates[:3]])
                return f"听起来你现在有点低落，可以先看看这些相对容易读进去的书：{picks}。如果你想，我也可以按治愈、轻松、科幻逃离感这几种方向继续帮你细分。"
            return "听起来你现在有点不开心。如果你愿意，我可以按治愈、轻松、科幻逃离感或思考人生这几种方向帮你推荐书。"
        if intent in {"cart_help", "cart_action"}:
            return "我已经识别到你的购物车操作意图。接下来会直接弹出确认框，只有你点击确认后，才会对当前会话用户的购物车执行变更。"
        ...
        if books:
            if len(books) == 1 and not self._wants_recommendation(message):
                ...
                parts = [
                    f"《{top['title']}》",
                    f"作者：{top.get('author') or top.get('author_en') or '暂无'}",
                    f"简介：{(top.get('description') or top.get('description_en') or '暂无').strip()}",
                ]
                return "；".join(parts)
            recommendations = []
            for item in books[:3]:
                reason = self._short_desc(item)
                recommendations.append(f"《{item['title']}》：{reason}")
            return "可以看看：" + "；".join(recommendations)
        ...
        if self._wants_recommendation(focus):
            ...
            for item in theme_candidates:
                ...
                if key in seen:
                    continue
                seen.add(key)
                picks.append(f"《{item['title']}》：{self._short_desc(item)}")
                if len(picks) >= 3:
                    break
            if picks:
                return "可以先看看这些：" + "；".join(picks)

        if self._is_book_related(message) and history:
            for item in reversed(history):
                content = item.get("content", "")
                if "《" in content and "》" in content:
                    return "如果你想基于上一条继续找同作者或同类型书，可以直接说作者名、题材或关键词。"
        if not self._is_book_related(message):
            return "我可以陪你聊天，也可以帮你查书、推荐书，或者说明如何通过接口完成购物车操作。"
        return "暂无相关信息"

    async def compose_cart_confirmation(self, action: str, product_title: str, quantity: int | None, option_summary: str | None) -> str:
        if not self.enabled:
            if action == "add":
